 ...
 *
 * Version 0.7
 * - 2015-06-06 <a.kihara@cgiar.org>: Add an option for adding timestamp to uploaded files
 *
 * Version 0.6
...
/**
    ...
    * @param	boolean  $appendTimestamp  (Optional) Whether to append a timestamp to the filename or not
    ...
    */
   static public function CustomSaveUploads($uploadDir, $html_name, $allowedTypes, $appendTimestamp = true, $max_size = 10485760, $filenames = null){
      $err_occ=0;
...
//            echo '<pre>'.print_r($_FILES[$html_name], true).'</pre>';
         ...
         if(isset($filenames)) $destfile = $uploadDir.$filenames[$i];
            ...
            if($appendTimestamp) $destfile = $uploadDir . pathinfo($_FILES[$html_name]['name'][$i], PATHINFO_FILENAME) . '_' . date('YmdHis') . '.' . pathinfo($_FILES[$html_name]['name'][$i], PATHINFO_EXTENSION);
