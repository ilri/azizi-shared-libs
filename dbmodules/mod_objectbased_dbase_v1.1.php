 ...
 *
 * Version 1.1
 *  - InsertOnDuplicateUpdate now returns dbcon->lastInsertId() instead of dbcon->insert_id which was returning a blank string
 *  - Introduced LDAP auth on the system
 *
 * Version 1.0
...
class DBase{
   ...
   public function InitializeConnection($config = NULL) {
      ...
      $this->CreateLogEntry('About to initialize database connection', 'info');
      $config = (is_null($config)) ? Config::$config : $config;

      try{
         $this->dbcon = new PDO("{$this->dbType}:dbname={$config['dbase']};host={$config['dbloc']}", $config['user'], $config['pass']);
   ...
   public function GetSingleRowValue($table, $toreturn, $col, $colval, $operand = "="){
      ...
      if(is_array($col)){
         ...
         for($i=0; $i<count($col); $i++){
            $colvals[$col[$i]] = $colval[$i];
            $con .= ( $i==0 ) ? "{$col[$i]} $operand :{$col[$i]}" : " AND {$col[$i]} $operand :{$col[$i]}";
         }
         $query = "SELECT $toreturn FROM $table WHERE $con LOCK IN SHARE MODE";
      }
      else{
         $colvals[$col] = $colval;
         $query = "SELECT $toreturn FROM $table WHERE $col $operand :$col LOCK IN SHARE MODE";
      }

      $res = $this->ExecuteQuery($query, $colvals, PDO::FETCH_NUM);
      if($res == 1) return -2;
      return $res[0][0];        //this is being returned as a string even if its an integer
   }
...
//      echo $this->query.'<br>';
   ...
   public function AddNewTray($dbase, $name, $size, $type, $keeper, $box_features = NULL, $location = NULL, $rack = NULL, $rack_position = NULL) {
      ...
      $colvals = array('box_name' => $name, 'size' => $size, 'box_type' => $type, 'keeper' => $keeper);
      $insertQuery = "insert into $dbase.boxes_def(box_name, size, box_type, keeper, box_features, location, rack, rack_position) values(:box_name, :size, :box_type, :keeper, :box_features, :location, :rack, :rack_position)";
      $updateQuery = "update $dbase.boxes_def set box_name = :box_name, size = :size, box_type = :box_type, keeper = :keeper, box_features = :box_features, location = :location, rack = :rack, rack_position = :rack_position where box_id = :box_id";
      $addons = array('box_features', 'location', 'rack', 'rack_position');

      foreach($addons as $t) {
         $colvals[$t] = (isset($$t)) ? $$t : NULL;       //associate the column name with the corresponding value
      }
      ...
      elseif(isset($res)) { //the box is already defined, so just update the data
         $boxId = (int)$res;  //convert it to an integer
         $colvals['box_id'] = $boxId;
         $res = $this->UpdateRecords($updateQuery, $colvals);
         if($res) return $this->lastError;
         //all is ok, return the last inserted id
         return $boxId;
      }
      elseif(is_null($res)) {  //we dont have this box so add it
         $res = $this->ExecuteQuery($insertQuery, $colvals);
         if($res) return $this->lastError;
         //all is ok, return the last inserted id
         return $this->dbcon->insert_id;
      }
   ...
   /**
    * Adds a new sample process to the labcollector's database.
    *
    * @param   string   $dbase        The name of the database that we are going to add the process to
    * @param   string   $processName  The name of the process to be added
    * @return  mixed    Return a string with the error message in case an error occurs, else it returns the inserted process id.
    * @since   v1.1
    */
   public function addProcessType($dbase, $processName) {
      if($processName=='' || !isset($processName)) return 'Cannot add an empty process name!!';
      //check if the sample type is already defined
      $processId = $this->GetSingleRowValue("$dbase.process_type_def", 'count', 'label', $processName);
      if($processId == -2) return $this->lastError;
      elseif(!is_null($processId)) return $processId;  //the sample is already added to the database

      $res = $this->UpdateRecords("insert into $dbase.process_type_def(label) values(:proc_type)", array('proc_type' => $processName));
   ...
   public function AddSpecies($dbase, $speciesName){
      ...
      $res = $this->UpdateRecords("insert into $dbase.organisms(org_name) values(:org)", array('org' => $speciesName));
      if($res == 1) return $this->lastError;
      return $res;
   }
   ...
   /**
    * Adds a new person as a contact to the database
    *
    * @param   string   $dbase        The name of the database that we are going to add the samples to
    * @param   string   $ownerName    The name of the person who is being added
    * @return  mixed    Return a string with the error message in case an error occurs, else it returns the inserted contact id.
    * @since   v1.1
    */
   public function AddOwner($dbase, $ownerName){
      if($ownerName == '' || !isset($ownerName)) return 'Cannot add a person without name!!';
      //check if the sample type is already defined
      $ownerId = $this->GetSingleRowValue("$dbase.contacts", 'count', 'name', $ownerName);
      if($ownerId == -2) return $this->lastError;
      elseif(!is_null($ownerId)) return $ownerId;  //the organism is already added to the database

      $res = $this->UpdateRecords("insert into $dbase.contacts(name) values(:name)", array('name' => $ownerName));
      if($res == 1) return $this->lastError;
      return $this->dbcon->insert_id;
   }
...
//      echo "$curLogFile---<br />";
   ...
   public function ConfirmUser($username, $password){
      ...
      $decryptedPW = $this->decryptRSACypherText($password);
      //in case we need an md5 of the password
      $md5pass = md5($decryptedPW);
...
//      echo $this->query;
      //update users set psswd=sha1(concat(salt,'ed95183d94836c9e353cdb36c807d71f'))  where login='solo.kihara'
//      echo "$username -- $md5pass -- $decryptedPW";
      $variables = array('username' => $username, 'password' => $md5pass);
...
//      echo print_r($result, true);
      if(count($result) == 0){
         ...
         if($res == 1) return 1;
         elseif(count($res) == 0){
            $ldap = $this->ADAuthenticate($username, $decryptedPW);
            if ($ldap == 1) return 1;
            elseif (is_string($ldap)) return $ldap;
            elseif ($ldap == 0) {
               $this->currentUserId = -1;
               $_SESSION['unhashedPW'] = $decryptedPW;
               $this->CreateLogEntry($_SESSION['username']." successfully logged in using AD", 'audit');
               return 0;
            }
         }
         ...
         $_SESSION['unhashedPW'] = $md5pass;
         $this->CreateLogEntry($_SESSION['onames']." ".$_SESSION['surname']." successfully logged in using local auth", 'audit');
         return 0;
      }
   }
...
//      echo '<br />Count -- '.count($res).'<br />';
    ...
    * @since   v0.5
    */
   public function ADAuthenticate($username, $password){
     ...
     if(!$this->ldapConnection){
         ...
         if (ldap_bind($this->ldapConnection, "$username@ilri.cgiarad.org", $password)) {
            ...
            $ldapSr = ldap_search($this->ldapConnection, Config::$config['ldapSSpace'], "(sAMAccountName=$username)", array('sn', 'givenName', 'title'));
            if (!$ldapSr) {
   ...
   public function GetCurrentUserDetails(){
      $query = "select a.id as user_id, a.sname, a.onames, a.login, b.name as user_type from " . Config::$config['session_dbase'] . ".users as a
               inner join " . Config::$config['session_dbase'] . ".user_levels as b on a.user_level=b.id  WHERE a.id = :id";

      $result = $this->ExecuteQuery($query, array('id' => $this->currentUserId));
      if ($result == 1) {
...
//            return 1;
    ...
    */
   public function decryptRSACypherText($cypherText) {
//      echo Config::$rsaPrivKey;
...
//      echo "<pre>manage session: ".print_r($this->session, true).'</pre>';
      ...
      if (!$ldapConnection) {
         ...
         if (!$ldapConnection)
            ...
            if (ldap_bind($ldapConnection, $_SESSION['username'] . "@ilri.cgiarad.org", $_SESSION['unhashedPW'])) {
               ...
               $ldapSr = ldap_search($ldapConnection, Config::$config['ldapSSpace'], "(sAMAccountName=$username)", array('mail'));
               if (!$ldapSr) {
   ...
   /**
    * Gets the title corresponding to the LDAP username specified
    *
    * @param   string   $username   LDAP username corresponding to the email address to be searched
    *
    * @return  mixed    Returns 0 if an error occures durind execution or the title corresponding to the email address
    */
   public function getUserTitle($username) {
      $ldapConnection = ldap_connect(Config::$config['ldapHost'], Config::$config['ldapPort']);
      if (!$ldapConnection) {
         return 0;
      } else {
         ...
         if (!$ldapConnection)
            ...
            if (ldap_bind($ldapConnection, $_SESSION['username'] . "@ilri.cgiarad.org", $_SESSION['unhashedPW'])) {
               ldap_set_option($ldapConnection, LDAP_OPT_REFERRALS, 0);
               ldap_set_option($ldapConnection, LDAP_OPT_PROTOCOL_VERSION, 3);
               $ldapSr = ldap_search($ldapConnection, Config::$config['ldapSSpace'], "(sAMAccountName=$username)", array('title'));
               if (!$ldapSr) {
                  $this->CreateLogEntry('', 'fatal');
                  return 0;
               }
   ...
   /**
    * Gets the full name (onames surname) corresponding to the LDAP username specified
    *
    * @param   string   $username   LDAP username corresponding to the email address to be searched
    *
    * @return  mixed    Returns 0 if an error occures durind execution or the full name (onames surname) corresponding to the email address
    */
      ...
      if (!$ldapConnection) {
         ...
         if (!$ldapConnection)
            ...
            if (ldap_bind($ldapConnection, $_SESSION['username'] . "@ilri.cgiarad.org", $_SESSION['unhashedPW'])) {
               ldap_set_option($ldapConnection, LDAP_OPT_REFERRALS, 0);
               ldap_set_option($ldapConnection, LDAP_OPT_PROTOCOL_VERSION, 3);
               $ldapSr = ldap_search($ldapConnection, Config::$config['ldapSSpace'], "(sAMAccountName=$username)", array('sn', 'givenName'));
               if (!$ldapSr) {
                  $this->CreateLogEntry('', 'fatal');
                  return 0;
               }
